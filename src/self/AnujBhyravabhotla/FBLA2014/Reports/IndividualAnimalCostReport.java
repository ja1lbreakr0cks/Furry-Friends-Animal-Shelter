...
public class IndividualAnimalCostReport {
	/**The main window frame*/
	private JFrame frmFurryFriendsAnimal;
	/**The table that displays the data for the inventory-wide cost report.  This table lists each cost and its dollar value.*/
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					IndividualAnimalCostReport window = new IndividualAnimalCostReport();
	 ...
	 */
	public IndividualAnimalCostReport() {
		initialize();
	}
	...
	private void initialize() {
		try{
				...
				public void actionPerformed(ActionEvent arg0) {
					try {
						table.print(PrintMode.FIT_WIDTH, new MessageFormat("Furry Friends Animal Shelter"),
								new MessageFormat("Cost Report for "+reportingAnimal.getName()));
			...
			lblSelectAnAnimal = new JLabel("Select an animal to see it's cost report: ");
			...
			animalSelector.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if(reportingAnimal!=null&&reportingAnimal.getType().equalsIgnoreCase("Dog"))
						table.setModel(new DefaultTableModel(Inventory.dogInventory.get(reportingAnimal.caseNumber).tabulateCost(),columnNames));
					if(reportingAnimal!=null&&reportingAnimal.getType().equalsIgnoreCase("Cat"))
						table.setModel(new DefaultTableModel(Inventory.catInventory.get(reportingAnimal.caseNumber).tabulateCost(),columnNames));
					if(reportingAnimal!=null&&reportingAnimal.getType().equalsIgnoreCase("Other"))
						table.setModel(new DefaultTableModel(reportingAnimal.tabulateCost(),columnNames));
				}
			});
			...
			JOptionPane.showMessageDialog(frmFurryFriendsAnimal, "The Inventory is Empty");
			frmFurryFriendsAnimal.setVisible(false);
			frmFurryFriendsAnimal.dispose();
			return;
		}
	}
